...
- Occlusion in `run_experiment.py` is a deterministic block of exactly
  L consecutive missed detections. `sim/stochastic_occlusion.py`
  relaxes this to a stochastic (Markov, geometric-duration) model and
  confirms the qualitative crossover result is preserved (Section 5.7
  of the paper), but only for a single occlusion episode per track;
  multiple/overlapping episodes and partial-fade dropout are not yet
  modeled.
