...
class MultivariateGaussianLibrary:
    """
    Multi-dimensional generalization of ClassLibrary. Each class is defined
    by a mean feature vector and a (possibly correlated) covariance matrix,
    representing e.g. a stack of ESM/RCS-derived channels (mean amplitude,
    amplitude spread, pulse-repetition-interval offset, ...) rather than a
    single scalar ID feature.

    Exposes the same two-method interface as ClassLibrary
    (sample_feature, log_likelihood), so CumulativeLLClassifier and
    EMAClassifier -- which only ever call these two methods and never
    assume scalar features -- work unmodified against this library.
    """

    def __init__(self, class_means, class_cov):
        """
        class_means: dict class_id -> np.ndarray, shape (d,)
        class_cov: either a single np.ndarray (d, d) shared across classes,
            or a dict class_id -> np.ndarray (d, d) for per-class covariance.
        """
        self.class_means = {c: np.asarray(m, dtype=float) for c, m in class_means.items()}
        self.class_ids = list(class_means.keys())
        self.d = next(iter(self.class_means.values())).shape[0]

        if isinstance(class_cov, dict):
            self.class_cov = {c: np.asarray(class_cov[c], dtype=float) for c in self.class_ids}
        else:
            shared = np.asarray(class_cov, dtype=float)
            self.class_cov = {c: shared for c in self.class_ids}

        self._chol = {c: np.linalg.cholesky(cov) for c, cov in self.class_cov.items()}
        self._inv = {c: np.linalg.inv(cov) for c, cov in self.class_cov.items()}
        self._logdet = {c: np.linalg.slogdet(cov)[1] for c, cov in self.class_cov.items()}

    def sample_feature(self, cls_id, rng):
        mean = self.class_means[cls_id]
        z = rng.normal(size=self.d)
        return mean + self._chol[cls_id] @ z

    def log_likelihood(self, feature_value, cls_id):
        mean = self.class_means[cls_id]
        diff = np.asarray(feature_value, dtype=float) - mean
        maha = diff @ self._inv[cls_id] @ diff
        return -0.5 * (self.d * np.log(2 * np.pi) + self._logdet[cls_id] + maha)
