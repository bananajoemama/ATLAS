...
Output:
    - Prints all summary statistics to stdout (matching the numbers
      quoted in Sections 5.3-5.7 of main.tex)
    - Writes summary_final.json (raw occlusion-sweep data) and
      summary_stochastic.json (raw stochastic-occlusion sweep data)
...
from stochastic_occlusion import sweep_stochastic_occlusion
...
def main():
    ...
    outcomes, atlas_acc_given_a, n_attempted, n_excluded = run_crossing_experiment(
    ...
    # ------------------------------------------------------------------
    # Section 5.7: stochastic occlusion robustness check
    # ------------------------------------------------------------------
    section_header("Section 5.7: Stochastic occlusion robustness")
    stochastic_summary = sweep_stochastic_occlusion(
        n_trials=200, tau_novel_ema=tau_ema, tau_novel_cum_norm=tau_cum
    )
    with open('summary_stochastic.json', 'w') as f:
        json.dump(stochastic_summary, f, indent=2)
    print("\n[saved summary_stochastic.json]")

    elapsed = time.time() - t0
    section_header(f"Done in {elapsed/60:.1f} minutes")
    print("All figures and summary_final.json / summary_stochastic.json written to the current directory.")
    print("Compare printed numbers above against Sections 5.3-5.7 of main.tex.")
